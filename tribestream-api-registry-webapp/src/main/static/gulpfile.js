...
// the directory where we put resources during the build (should it be ./build instead?)
var workdir = '../../../target/static-resources/';
...
gulp.task('css-third-party', function () {
    var regular = merge(
            ...
            gulp.src(['./bower_components/simplemde/dist/simplemde.min.css'])
                .pipe(cssWrap({selector:'.simplemde'}))
        )
        .pipe(concat('_.css'))
        .pipe(gulp.dest(workdir + 'app/third-party/styles/'));

    es.concat(
        regular,
        gulp.src(['./bower_components/mocha/mocha.css']).pipe(concat('_tests.css')).pipe(gulp.dest(workdir + 'app/third-party/styles/')));
});
gulp.task('css-third-party-resources', function () {
    var font = gulp.src([
        './bower_components/lato/font/**/*'
    ]).pipe(gulp.dest(workdir + 'app/third-party/font'));
    var fonts = gulp.src([
        './bower_components/montserrat-webfont/fonts/**/*',
        './bower_components/open-sans/fonts/**/*',
        './bower_components/font-awesome/fonts/*'
    ]).pipe(gulp.dest(workdir + 'app/third-party/fonts'));
    var foundationTemplates = gulp.src([
        './bower_components/foundation-apps/js/angular/components/**/*.html'
    ]).pipe(gulp.dest(workdir + 'components'));
...
gulp.task('autoprefixer', function () {
    return gulp.src(workdir + 'app/styles/app.css')
        .pipe(autoprefixer({
            browsers: ['last 4 versions']
        }))
        .pipe(gulp.dest(workdir + 'app/styles/'));
});
gulp.task('css-concat', function () {
    return gulp.src([workdir + 'app/styles/sprite.css', workdir + 'app/styles/app.css'])
        .pipe(concat('_.css'))
        .pipe(gulp.dest(workdir + 'app/styles/'))
});
...
gulp.task('copy-templates', function () {
    var asScripts =  gulp.src('../../../target/static-templates/_templates.js')
        .pipe(gulp.dest(workdir + 'app/scripts/'));

    var asTemplates =  gulp.src('../../../target/static-templates/html/templates/*')
        .pipe(gulp.dest(workdir + 'app/templates/'));
...
gulp.task('js-third-party-test', function () {
    ...
    ]).pipe(concat('_tests_1.js')).pipe(gulp.dest(workdir + 'app/third-party/'));
    var _2 = gulp.src([
    ...
    ]).pipe(concat('_tests_2.js')).pipe(gulp.dest(workdir + 'app/third-party/'));
